...
class Import:
    def __init__(self, _import="", _as = None, _from=""):
        self._import = _import
        self._as     = _as
        self._from   = _from
    def __repr__(self):
        if self._as:
            return f'import {self._import} as {self._as} from "{self._from}";'
        else:
            return f'import {self._import} from "{self._from}";'
...
class Component:
    def __init__(self):
        ...
        self._methods = []
        self._return  = []
    def _set_imports(self, _imports):
        for _import in _imports:
            self._imports.append(Import(_import[0], _import[1], _import[2] ))
...
def main():
    ...
    for counter, arg in enumerate(sys.argv):
        ...
        if arg=="debug":
            pprint(proj.components)
            pprint(proj.reducers)
        
        if arg=="help":
            print('''            commands:
            python cli.py create-component [component name]
            python cli.py create-reducer [reducer name]
            python cli.py create-action [action name] [reducer name] [OPTIONS: -stateful]
            ''')
